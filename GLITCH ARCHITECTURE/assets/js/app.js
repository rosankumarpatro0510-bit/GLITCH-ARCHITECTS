...
(function (global) {
  ...
  function shell() {
    ...
    <aside class="rail" id="rail">
      <a class="rail__brand" href="index.html">
        ...
        <span><b>Glitch Architecture</b><small> Severe-Weather Nowcasting </small></span>
      </a>
      <nav class="rail__nav">
  ...
  async function ask(q, spoken) {
    if (!q.trim()) return;
    log('you', U.esc(q));
    const pending = log('bot', '<span class="src">Connecting to the local Gemini assistant…</span>');
    let html;
    try {
      html = `<b>${U.esc(await A.answerWithApi(q))}</b><span class="src">Generated by the configured local Gemini backend.</span>`;
    } catch (error) {
      console.warn('Assistant API unavailable; using local assistant.', error);
      html = A.answer(q);
      html += `<span class="src">Local Gemini backend unavailable, so this answer uses the built-in offline assistant.</span>`;
    }
